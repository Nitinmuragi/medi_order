...
class Med_User(UserMixin, db.Model):
    __tablename__ = 'users'
    ...
    # Relationships
    orders = db.relationship('Med_Order', backref='user', lazy=True)
    prescriptions = db.relationship('Med_Prescription', backref='user', lazy=True)
    notifications = db.relationship('Med_Notification', backref='user', lazy=True)

class Med_Store(db.Model):
    ...
    # Relationships
    orders = db.relationship('Med_Order', backref='store', lazy=True)
    
    # Relationship with user
    user = db.relationship('Med_User', backref=db.backref('store_profile', uselist=False))

class Med_Prescription(db.Model):
    ...
    # Relationships
    orders = db.relationship('Med_Order', backref='prescription', lazy=True)

class Med_Order(db.Model):
...
db.Index('idx_orders_user_id', Med_Order.user_id)
db.Index('idx_orders_store_id', Med_Order.store_id)
db.Index('idx_orders_tracking_id', Med_Order.tracking_id)
db.Index('idx_prescriptions_user_id', Med_Prescription.user_id)