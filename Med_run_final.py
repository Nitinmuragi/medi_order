...
from flask import (
    Flask, render_template, render_template_string, redirect, url_for, flash,
...
from Med_app.Med_models import (
    Med_User, Med_Store, Med_Prescription, Med_Order,
...
from Med_app import db, login_manager, migrate

# create the Flask application
app = Flask(__name__, template_folder="Med_app/templates", static_folder="Med_app/static")
app.config.from_object(Med_Config)

# enable debug logging for troubleshooting
app.logger.setLevel('DEBUG')

# initialise extensions (objects imported from Med_app package)
db.init_app(app)
...
def Med_send_email(to_email, subject, html_body):
...
def Med_create_razorpay_payment_link(order):
    ...
    try:
        payment_link_data = {
            ...
            "description": f"Med_Payment for Med_Order #{order.tracking_id}",
            "customer": {
            ...
            "callback_url": url_for('Med_payment.Med_razorpay_callback', order_id=order.id, _external=True),
            "callback_method": "get"
        }

        payment_link = razorpay_client.payment_link.create(payment_link_data)

        # Med_Store payment link in database
        payment = Med_Payment(
            order_id=order.id,
...
def Med_send_order_confirmation_email(order):
    """Send order confirmation email to customer"""
    app.logger.info("=" * 60)
    app.logger.info("🔔 TRIGGERED: Med_send_order_confirmation_email")
    app.logger.info(f"Med_Order ID: {order.id}")
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>🎉 Med_Order Confirmed!</h1>
                <p>Your medicine order has been placed successfully</p>
            </div>
            <div class="content">
                <div class="order-id">
                    <p style="margin: 0; color: #6b7280; font-size: 14px;">Med_Order ID</p>
                    <h2>{order.tracking_id}</h2>
                </div>

                <div class="details">
                    <h3 style="margin-top: 0;">📋 Med_Order Details</h3>
                    <div class="detail-row">
                        <span><strong>Med_Store:</strong></span>
                    ...
                    <div class="detail-row">
                        <span><strong>Med_Payment Method:</strong></span>
                        <span>{order.payment_method}</span>
                    </div>
                    <div class="detail-row">
                        <span><strong>Status:</strong></span>
                        <span style="color: #16a34a; font-weight: 600;">Med_Order Placed</span>
                    </div>
                </div>
    ...
    result = Med_send_email(
        order.user.email,
        f"Med_Order Confirmed - {order.tracking_id}",
        html_body
    )
    
    if result:
        app.logger.info(f"✅ Med_Order confirmation email sent to {order.user.email}")
    else:
        app.logger.error(f"❌ Failed to send order confirmation email to {order.user.email}")


def Med_send_order_accepted_email(order, payment_link=None):
    """Send order accepted email to customer with payment link"""
    app.logger.info("=" * 60)
    app.logger.info("🔔 TRIGGERED: Med_send_order_accepted_email")
    app.logger.info(f"Med_Order ID: {order.id}")
    app.logger.info(f"Customer: {order.user.full_name}")
    app.logger.info(f"Customer Email: {order.user.email}")
    app.logger.info(f"Med_Payment Method: {order.payment_method}")
    app.logger.info(f"Med_Payment Link: {payment_link}")
    app.logger.info("=" * 60)
    ...
    if order.payment_method == 'ONLINE' and payment_link:
        ...
        <div style="background: #eff6ff; border: 2px solid #3b82f6; border-radius: 8px; padding: 20px; margin: 20px 0; text-align: center;">
            <h3 style="color: #1e40af; margin-top: 0;">💳 Med_Payment Required</h3>
            ...
            <p style="font-size: 12px; color: #6b7280; margin-top: 15px;">
                Med_Payment link expires in 24 hours
            </p>
        </div>
        """
    else:
    ...
    <body>
        <div class="container">
            ...
            <div class="content">
                ...
                <div class="details">
                    <h3 style="margin-top: 0;">📋 Med_Order Summary</h3>
                    <div class="detail-row">
                ...
                <div style="background: #f0fdf4; border-left: 4px solid #16a34a; padding: 15px; margin: 20px 0;">
                    <p style="margin: 0; color: #14532d;">
                        <strong>🏪 {store.name if store else 'Med_Store'}</strong><br>
    ...
    result = Med_send_email(
        order.user.email,
        f"Med_Order Accepted - Med_Payment {order.payment_method} - {order.tracking_id}",
        html_body
    )
    
    if result:
        app.logger.info(f"✅ Med_Order accepted email sent to {order.user.email}")
    else:
        app.logger.error(f"❌ Failed to send order accepted email to {order.user.email}")


def Med_send_store_new_order_email(order, store):
    """Send new order notification to store"""
    if not store or not store.user or not store.user.email:
        app.logger.warning(f"Med_Store or store user email missing for order {order.id}")
        return
    
    app.logger.info("=" * 60)
    app.logger.info("🔔 TRIGGERED: Med_send_store_new_order_email")
    app.logger.info(f"Med_Order ID: {order.id}")
    app.logger.info(f"Med_Store: {store.name}")
    app.logger.info(f"Med_Store Email: {store.user.email}")
    app.logger.info("=" * 60)
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>🔔 New Med_Order Received!</h1>
                <p>A customer has placed a new order</p>
            </div>
            <div class="content">
                <div class="alert">
                    <h2 style="margin: 0; color: #713f12;">Med_Order #{order.tracking_id}</h2>
                ...
                <div style="text-align: center;">
                    <a href="{url_for('Med_store.Med_store_dashboard', _external=True)}" class="btn">
                        View Med_Order & Accept
                    </a>
                </div>
            ...
            <div class="footer">
                <p>© 2026 MediOrder - Med_Store Management Portal</p>
            </div>
        </div>
    </body>
    </html>
    ...
    result = Med_send_email(
        store.user.email,
        f"🔔 New Med_Order #{order.tracking_id}",
        html_body
    )
    
    if result:
        app.logger.info(f"✅ Med_Store notification email sent to {store.user.email}")
    else:
...
def Med_calculate_distance(lat1, lon1, lat2, lon2):
...
def Med_find_nearest_stores(user_lat, user_lon, limit=10):
    """Find nearest stores to user location"""
    if not user_lat or not user_lon:
        # If no user location, return all active stores
        return Med_Store.query.filter_by(is_active=True).all()

    # Get all active stores
    stores = Med_Store.query.filter_by(is_active=True).all()

    # Calculate distance for each store
    for store in stores:
        store.distance = Med_calculate_distance(
            user_lat, user_lon,
...
def Med_find_nearest_store(user_lat, user_lon):
    """Find single nearest store to user location"""
    stores = Med_find_nearest_stores(user_lat, user_lon, limit=1)
    if stores:
        store = stores[0]
...
def Med_reverse_geocode_google(lat, lng):
    """Resolve coordinates into a formatted address using Google Geocoding API."""
    api_key = os.getenv('GOOGLE_MAPS_API_KEY', '').strip()
    if not api_key:
        return None

    params = urlencode({'latlng': f'{lat},{lng}', 'key': api_key})
    ...
    try:
        ...
        app.logger.warning(f"Google reverse geocode failed: {exc}")
        return None

    if payload.get('status') != 'OK' or not payload.get('results'):
        app.logger.info(f"Google reverse geocode status: {payload.get('status')}")
        return None
    ...
    for comp in result.get('address_components', []):
        ...
        if not city and ('locality' in comp_types or 'administrative_area_level_2' in comp_types):
            city = comp.get('long_name', '')
        if 'postal_code' in comp_types:
...
def Med_index():
    if current_user.is_authenticated:
        if current_user.role == "customer":
            return redirect(url_for('Med_user.Med_dashboard'))
        elif current_user.role == "store":
            return redirect(url_for('Med_store.Med_store_dashboard'))
        elif current_user.role == "admin":
            return redirect(url_for('Med_admin.Med_admin_dashboard'))
...
Med_auth_bp = Blueprint("Med_auth", __name__, url_prefix="/Med_auth")
Med_user_bp = Blueprint("Med_user", __name__, url_prefix="/Med_user")
Med_store_bp = Blueprint("Med_store", __name__, url_prefix="/Med_store")
Med_order_bp = Blueprint("Med_order", __name__, url_prefix="/Med_order")
Med_payment_bp = Blueprint("Med_payment", __name__, url_prefix="/Med_payment")
Med_admin_bp = Blueprint("Med_admin", __name__, url_prefix="/Med_admin")

# =========================
# auth routes
# =========================

@Med_auth_bp.route("/Med_register", methods=["GET", "POST"])
def Med_register():
    if request.method == "POST":
        ...
        existing_user = Med_User.query.filter_by(phone=phone).first()
        if existing_user:
            flash("Phone number already registered!", "danger")
            return redirect(url_for('Med_auth.Med_register'))

        new_user = Med_User(
            full_name=full_name,
            phone=phone,
            email=email,
            password_hash=Med_hash_password(password),
            role=role
        )
        ...
        flash("Registration successful! Please Med_login.", "success")
        return redirect(url_for('Med_auth.Med_login'))

    return render_template("Med_register.html")


@Med_auth_bp.route("/Med_login", methods=["GET", "POST"])
def Med_login():
    if request.method == "POST":
        phone = request.form.get("phone")
        password = request.form.get("password")
    ...
    return render_template("Med_login.html")


@Med_auth_bp.route("/Med_logout")
@login_required
def Med_logout():
    logout_user()
...
@Med_user_bp.route("/Med_dashboard")
@login_required
def Med_dashboard():
    ...
    orders = (
        Med_Order.query
         ...
         .order_by(Med_Order.created_at.desc())
         .all()
    )

    return render_template("Med_dashboard_user.html", orders=orders)


@Med_user_bp.route("/Med_new-order")
@login_required
def Med_new_order():
    if current_user.role != "customer":
    ...
    if user_lat and user_lon:
        stores = Med_find_nearest_stores(user_lat, user_lon, limit=10)
    else:
...
@Med_store_bp.route("/Med_dashboard")
@login_required
def Med_store_dashboard():
    if current_user.role != "store":
        flash("Access denied!", "danger")
        return redirect(url_for('Med_auth.Med_login'))

    # Get store profile for this user
    store = Med_Store.query.filter_by(user_id=current_user.id).first()

    # Get orders for this store
    if store:
        orders = Med_Order.query.filter_by(store_id=store.id)\
                           .order_by(Med_Order.created_at.desc()).all()
    else:
        orders = []

    save_success = request.args.get('save_success', False)

    return render_template("Med_medical_dashboard.html",
                         orders=orders,
                         store=store,
                         save_success=save_success)


@Med_store_bp.route("/Med_save-profile", methods=["POST"])
@login_required
def Med_save_profile():
    ...
    try:
        ...
        if not store:
            ...
            store = Med_Store(user_id=current_user.id)
        ...
        flash("Med_Store profile saved successfully!", "success")
        return redirect(url_for('Med_store.Med_store_dashboard', save_success=True))
...
def Med_admin_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # role is stored in lowercase in the Med_User model
...
@Med_admin_bp.route("/Med_stores")
@login_required
@Med_admin_required
def Med_list_stores():
    stores = Med_Store.query.all()
    return render_template("Med_admin_stores.html", stores=stores)


@Med_store_bp.route("/Med_accept/<int:order_id>", methods=["POST"])
@login_required
def Med_accept_order(order_id):
    if current_user.role != "store":
        flash("Access denied!", "danger")
        return redirect(url_for('Med_auth.Med_login'))

    order = Med_Order.query.get_or_404(order_id)
    ...
    if order.payment_method == 'ONLINE' and is_payment_enabled():
        payment_link = Med_create_razorpay_payment_link(order)

    # Send email to customer with payment link
    if order.user.email:
        Med_send_order_accepted_email(order, payment_link)

    flash("Med_Order accepted successfully! Customer notified via email.", "success")
    return redirect(url_for('Med_store.Med_store_dashboard'))


@Med_store_bp.route("/Med_reject/<int:order_id>", methods=["POST"])
@login_required
def reject_order(order_id):
    ...
    flash("Med_Order rejected!", "warning")
    return redirect(url_for('Med_store.Med_store_dashboard'))


@Med_store_bp.route("/Med_update_status/<int:order_id>", methods=["POST"])
@login_required
def update_status(order_id):
    ...
    flash("Med_Order status updated!", "success")
    return redirect(url_for('Med_store.Med_store_dashboard'))


@Med_store_bp.route("/Med_mark_paid/<int:order_id>", methods=["POST"])
@login_required
...
@Med_store_bp.route("/Med_update_price/<int:order_id>", methods=["POST"])
@login_required
...
@Med_store_bp.route("/Med_process/<int:order_id>", methods=["POST"])
@login_required
def process_order(order_id):
    """API used by the enhanced Med_dashboard modal to accept and price an order."""
    if current_user.role != "store":
        return ("", 403)
...
@Med_order_bp.route("/Med_track", methods=["GET", "POST"])
def Med_track_order():
    order = None
    if request.method == "POST":
        tracking_id = request.form.get("tracking_id")
        order = Med_Order.query.filter_by(tracking_id=tracking_id).first()
    return render_template("Med_order_tracking.html", order=order)


@Med_order_bp.route("/Med_reverse-geocode", methods=["POST"])
@login_required
def Med_reverse_geocode():
    if current_user.role != "customer":
    ...
    geocoded = Med_reverse_geocode_google(lat, lng)
    if not geocoded:
        return jsonify({
            "ok": True,
            "formatted_address": "",
    ...
    return jsonify({"ok": True, **geocoded})


@Med_order_bp.route("/Med_create", methods=["GET","POST"])
@login_required
def Med_create_order():
    if current_user.role != "customer":
        flash("Only customers can create orders!", "danger")
        return redirect(url_for('Med_auth.Med_login'))
    ...
    if request.method == "POST":
        ...
        user_map_link = request.form.get("user_map_link")

        parsed_user_lat = None
        parsed_user_lng = None
        try:
            if user_lat and user_lng:
                parsed_user_lat = float(user_lat)
                parsed_user_lng = float(user_lng)
        except (TypeError, ValueError):
            parsed_user_lat = None
            parsed_user_lng = None
        ...
        if not file:
            ...
            return redirect(url_for('Med_user.Med_new_order'))
        ...
        if not store_id:
            if parsed_user_lat is not None and parsed_user_lng is not None:
                nearest_store, delivery_charge = Med_find_nearest_store(
                    parsed_user_lat, parsed_user_lng
                )
            else:
                nearest_store, delivery_charge = Med_find_nearest_store(
        ...
        if not payment_method:
            ...
            flash("Online payment is currently unavailable. Med_Order placed with Cash on Delivery.", "info")
        ...
        if store_id:
            selected_store = Med_Store.query.get(int(store_id))
            delivery_charge = selected_store.delivery_charge if selected_store else 50.0
        else:
            if parsed_user_lat is not None and parsed_user_lng is not None:
                selected_store, delivery_charge = Med_find_nearest_store(parsed_user_lat, parsed_user_lng)
            else:
                selected_store, delivery_charge = Med_find_nearest_store(
                    getattr(current_user, 'latitude', None),
                    getattr(current_user, 'longitude', None)
                )

        # Save latest detected location on user profile for better nearest-store suggestions next time.
        if parsed_user_lat is not None and parsed_user_lng is not None:
            current_user.latitude = parsed_user_lat
            current_user.longitude = parsed_user_lng

        if user_map_link and user_map_link.startswith("http"):
            if "Open map:" not in (delivery_address or ""):
                delivery_address = f"{delivery_address} | Open map: {user_map_link}"
        ...
        order = Med_Order(
            ...
            delivery_lat=parsed_user_lat,
            delivery_lng=parsed_user_lng,
        ...
        app.logger.info("=" * 60)
        app.logger.info(f"Med_Order created successfully: {tracking_id}")
        ...
        if current_user.email:
            ...
            Med_send_order_confirmation_email(order)
        else:
            app.logger.warning(f"⚠️ Med_User {current_user.id} has no email address!")
        ...
        if selected_store:
            ...
            Med_send_store_new_order_email(order, selected_store)

        flash(f"Med_Order created successfully! Tracking ID: {tracking_id}", "success")
        return redirect(url_for('Med_user.Med_dashboard'))
...
@Med_payment_bp.route("/Med_pay/<int:order_id>")
@login_required
def Med_pay(order_id):
    order = Med_Order.query.get_or_404(order_id)
...
@Med_payment_bp.route("/Med_razorpay/Med_callback")
def Med_razorpay_callback():
    ...
    try:
        ...
        order = Med_Order.query.get(int(order_id))

        if razorpay_payment_link_status == 'paid':
            # Update payment status
            payment = Med_Payment.query.filter_by(order_id=order.id).first()
            if payment:
            ...
            flash("Med_Payment successful! Your order is being prepared.", "success")
        else:
            flash("Med_Payment failed or cancelled. Please try again.", "danger")

        return redirect(url_for('Med_user.Med_dashboard'))

    except Exception as e:
        app.logger.error(f"Razorpay callback error: {str(e)}")
        flash("Med_Payment processing error. Please contact support.", "danger")
        return redirect(url_for('Med_user.Med_dashboard'))


@Med_payment_bp.route("/Med_razorpay/Med_webhook", methods=["POST"])
def Med_razorpay_webhook():
    ...
    try:
        ...
        if event == 'payment_link.paid':
            ...
            # Find payment record
            payment = Med_Payment.query.filter_by(transaction_id=payment_link_id).first()
            if payment:
                payment.status = 'SUCCESS'
                ...
                app.logger.info(f"Med_Payment successful for order {order.tracking_id}")
...
@Med_admin_bp.route("/Med_dashboard")
@login_required
@Med_admin_required
def Med_admin_dashboard():
    from datetime import datetime, timedelta

    # Chart-only date-range quick filter
    valid_days = {7, 30, 180}
    try:
        chart_days = int(request.args.get("chart_days", 180))
    except (TypeError, ValueError):
        chart_days = 180
    if chart_days not in valid_days:
        chart_days = 180

    now = datetime.utcnow()

    # Basic counts
    total_users = Med_User.query.filter_by(role="customer").count()
    total_stores = Med_Store.query.count()
    total_orders = Med_Order.query.count()

    # Revenue (only successful payments)
    revenue_result = db.session.query(func.sum(Med_Order.total_amount)).filter(
        Med_Order.payment_status == "SUCCESS"
    ).scalar()
    revenue = float(revenue_result or 0.0)

    # Recent transactions (latest 4)
    recent_orders = (
        Med_Order.query
        .options(db.joinedload(Med_Order.user), db.joinedload(Med_Order.store))
        .order_by(Med_Order.created_at.desc())
        .limit(4)
        .all()
    )
    ...
    active_users = (
        db.session.query(func.count(func.distinct(Med_Order.user_id)))
        .filter(Med_Order.created_at >= last_30)
        .scalar() or 0
    )
    ...
    if chart_days in (7, 30):
        chart_range_label = f"Last {chart_days} days"
        for i in range(chart_days - 1, -1, -1):
            ...
            day_rev = (
                db.session.query(func.sum(Med_Order.total_amount))
                .filter(
                    Med_Order.created_at >= day_start,
                    Med_Order.created_at < day_end,
                    Med_Order.payment_status == "SUCCESS"
                )
                .scalar() or 0
            )
            ...
            day_count = (
                db.session.query(func.count(Med_Order.id))
                .filter(Med_Order.created_at >= day_start, Med_Order.created_at < day_end)
                .scalar() or 0
            )

            revenue_list.append(float(day_rev))
            orders_list.append(int(day_count))
    else:
        chart_range_label = "Last 6 months"
        for i in range(5, -1, -1):
            ...
            month_rev = (
                db.session.query(func.sum(Med_Order.total_amount))
                .filter(
                    Med_Order.created_at >= start,
                    Med_Order.created_at < end,
                    Med_Order.payment_status == "SUCCESS"
                )
                .scalar() or 0
            )
            ...
            month_count = (
                db.session.query(func.count(Med_Order.id))
                .filter(Med_Order.created_at >= start, Med_Order.created_at < end)
                .scalar() or 0
            )

            revenue_list.append(float(month_rev))
            orders_list.append(int(month_count))
    ...
    top_stores_q = (
        db.session.query(
            Med_Store.id,
            Med_Store.name,
            func.sum(Med_Order.total_amount).label("rev"),
            func.count(Med_Order.id).label("orders")
        )
        .join(Med_Order, Med_Order.store_id == Med_Store.id)
        .filter(Med_Order.created_at >= month_start)
        .group_by(Med_Store.id, Med_Store.name)
        .order_by(func.sum(Med_Order.total_amount).desc())
        .limit(5)
        .all()
    )

    top_stores = []
    for s in top_stores_q:
        top_stores.append({
            "id": s.id,
            "name": s.name,
    ...
    activities = []
    notifs = Med_Notification.query.order_by(Med_Notification.created_at.desc()).limit(5).all()
    for n in notifs:
        activities.append({
            "type": "notification",
            "message": n.message,
    ...
    for o in recent_orders[:5]:
        activities.append({
            "type": "order",
            "message": f"Med_Order {o.tracking_id} - {o.order_status}",
            "time": o.created_at
        })

    # Safe Med_logout URL detection
    logout_endpoint = None
    if "Med_auth.Med_logout" in current_app.view_functions:
        logout_endpoint = "Med_auth.Med_logout"
    elif "user.Med_logout" in current_app.view_functions:
        logout_endpoint = "user.Med_logout"
    elif "auth.Med_logout" in current_app.view_functions:
        logout_endpoint = "auth.Med_logout"
    elif "admin.Med_logout" in current_app.view_functions:
        logout_endpoint = "admin.Med_logout"

    return render_template(
        "Med_dashboard_admin.html",
        ...
        monthly_values=revenue_list,
        monthly_orders=orders_list,
...
@Med_admin_bp.route("/Med_order/<int:order_id>")
@login_required
@Med_admin_required
def Med_view_order(order_id):
    order = Med_Order.query.get_or_404(order_id)
    return render_template("admin/Med_view_order.html", order=order)


@Med_admin_bp.route("/Med_manage-users")
@login_required
@Med_admin_required
def Med_manage_users():
    users = Med_User.query.filter_by(role="customer").all()
    return render_template("admin/Med_manage_users.html", users=users)

@Med_admin_bp.route("/users/<int:user_id>/view-orders")
@login_required
@Med_admin_required
def Med_view_user_orders(user_id):
    """View all orders for a specific user"""
    user = Med_User.query.get_or_404(user_id)
    if user.role != "customer":
        flash("Invalid user", "danger")
        return redirect(url_for('Med_admin.Med_manage_users'))

    user_orders = Med_Order.query.filter_by(user_id=user_id).order_by(Med_Order.created_at.desc()).all()
    admin_summary = {
        "total_orders": len(user_orders),
        "delivered_orders": sum(1 for order in user_orders if order.order_status == "DELIVERED"),
        "pending_orders": sum(1 for order in user_orders if order.order_status in ["NEW", "ACCEPTED", "PREPARING", "OUT_FOR_DELIVERY"]),
        "cancelled_orders": sum(1 for order in user_orders if order.order_status in ["CANCELLED", "REJECTED"]),
        "total_amount": sum((order.total_amount or 0) for order in user_orders)
    }

    return render_template(
        "admin/Med_view_user_orders.html",
        user=user,
        user_orders=user_orders,
        admin_summary=admin_summary
    )

@Med_admin_bp.route("/users/<int:user_id>/delete", methods=["POST"])
@login_required
@Med_admin_required
def Med_delete_user(user_id):
    """Delete a user"""
    user = Med_User.query.get_or_404(user_id)
    ...
    try:
        ...
        Med_Order.query.filter_by(user_id=user_id).delete()
        # Delete related prescriptions
        Med_Prescription.query.filter_by(user_id=user_id).delete()
        # Delete the user
...
@Med_admin_bp.route("/orders/<int:order_id>/delete", methods=["POST"])
@login_required
@Med_admin_required
def Med_delete_order(order_id):
    """Delete an order"""
    order = Med_Order.query.get_or_404(order_id)
    ...
    try:
        ...
        db.session.commit()
        flash(f"Order '{tracking_id}' has been successfully deleted!", "success")
    except Exception as e:
        db.session.rollback()
        flash(f"Error deleting order: {str(e)}", "danger")
    
    return redirect(url_for('Med_admin.Med_view_user_orders', user_id=user_id))

@Med_admin_bp.route("/Med_manage-stores")
@login_required
@Med_admin_required
def Med_manage_stores():
    stores = Med_Store.query.all()
    return render_template("admin/Med_stores.html", stores=stores)


# (removed) Temporary HTMX test route — cleanup complete


@Med_admin_bp.route("/Med_approve-store/<int:store_id>", methods=["POST"])
@login_required
@Med_admin_required
def Med_approve_store(store_id):
    store = Med_Store.query.get_or_404(store_id)
    store.is_active = True
    db.session.commit()
    flash(f"Med_Store {store.name} has been approved!", "success")
    return redirect(url_for('Med_admin.Med_manage_stores'))


@Med_admin_bp.route("/Med_reject-store/<int:store_id>", methods=["POST"])
@login_required
@Med_admin_required
def Med_reject_store(store_id):
    store = Med_Store.query.get_or_404(store_id)
    store.is_active = False
    db.session.commit()
    flash(f"Med_Store {store.name} has been rejected!", "warning")
    return redirect(url_for('Med_admin.Med_manage_stores'))

# route for serving uploaded files
@app.route("/Med_uploads/<path:filename>")
def Med_uploaded_file(filename):
    return send_from_directory(current_app.config['UPLOAD_FOLDER'], filename)

# ================================================================
# ADD THIS ROUTE to your Med_store_bp section in routes.py
# Place it after the existing process_order route
# ================================================================

@Med_store_bp.route("/Med_send_bill/<int:order_id>", methods=["POST"])
@login_required
def Med_send_bill(order_id):
    """
    ...
    try:
        ...
        store_name      = data.get("store_name", "Medical Med_Store")
                                        ...
                                        <tr>
                                            <td style="padding:14px 18px; border-right:1px solid #bbf7d0;">
                                                <p style="margin:0; font-size:11px; color:#6b7280;
                                                           text-transform:uppercase; letter-spacing:1px;">Med_Order ID</p>
                                            ...
                                            <td style="padding:14px 18px;">
                                                <p style="margin:0; font-size:11px; color:#6b7280;
                                                           text-transform:uppercase; letter-spacing:1px;">Med_Payment</p>
...
@Med_user_bp.route("/Med_track", methods=['GET'])
@login_required
def Med_track_order():
    ...
    return render_template('Med_track_order.html',
        order=order, tracking_id=tracking_id, error=error)

# Med_register blueprints on app
app.register_blueprint(Med_auth_bp)
app.register_blueprint(Med_user_bp)
app.register_blueprint(Med_store_bp)
app.register_blueprint(Med_order_bp)
